	...
	*ESTIMATION OF THE IMPACT OF KCGF ON FIRMS' PRODUCTIVITY, SALES, EMPLOYMENT, AND PROBABILITY OF STOPPING THEIR OPERATIONS
	...
	-> Comparison Group 0 	-> Firms with no loans at all. Estimate of the impact of KCGF comparing KCGF firms and firms without loans
	-> Comparison Group 1 	-> Firms with other types of loans. Estimate of the impact of KCGF comparing KCGF firms and firms with other loans
	...
	*Propensity score matching using the vector of covariates suggested by the machine learning and testing parallel trends of the matched sample
	/*
	Next step is to match treated and comparison firms testing all the vectors of covariates listed above. 
	*/
	*________________________________________________________________________________________________________________________________________*
	{
	...
	*________________________________________________________________________________________________________________________________________*
	{
		...
		replace model = 8 if variable == 4 & model == 4
			...
			*Sales and productivity
						...
						ysize(4) xsize(7)  ///
						note("", color(black) fcolor(background) pos(7) size(small)) 
						graph export "$output/figures/results_preliminary_analyis1.pdf", as(pdf) replace
			...
			*Employees and closing
						...
						ysize(4) xsize(7)  ///
						note("", color(black) fcolor(background) pos(7) size(small)) 
						graph export "$output/figures/results_preliminary_analysis2.pdf", as(pdf) replace
		}				
	...
	*________________________________________________________________________________________________________________________________________*
		...
		foreach model in ols lasso ridge forest xgboost 	{  //ols lasso ridge  forest xgboost
			...
			use "$data\final\firm_year_level.dta" if period == 2015, clear
				...
				merge m:1 fuid using  "$data\inter\matching_models\matching_`model'_1.dta", keep(3) nogen keepusing(_weight _weight2 _support) //there are no significant differences between treatment and comparison groups on the matched sample
				gen micro =  sme == "a.1-9" 
