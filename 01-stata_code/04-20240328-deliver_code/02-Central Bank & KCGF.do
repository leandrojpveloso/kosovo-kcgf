	...
	*LOAN DATASETS
		...
		**
		** Credit Registry Data -> each row is a loan ID for firm i, in year t. Therefore, the same firm can have several loans in the same year
		*------------------------------------------------------------------------------------------------------------------------------------->>
		{
			**
			*Importing
			*------------------------------------------------------------------------------------------------------------------------------------->>
			/***
			import excel using  "${path_project}/data/raw\LoanApplications-2010-2018.xlsx", clear firstrow 
				duplicates r _all
				duplicates drop
				save "${path_project}/data/02-inter\Credit Registry Raw.dta", replace
			*/	 
			
			**
			**Cleaning
			*------------------------------------------------------------------------------------------------------------------------------------->>
			use "${path_project}/data/02-inter\Credit Registry Raw.dta", clear
				...
				*--------------------------------------------------------------------------------------------------------------------------------->>
				merge 		m:1 fuid period using "${path_project}/data/02-inter\Tax Registry.dta", keepusing(group_sme) keep (1 3)
				...
				*--------------------------------------------------------------------------------------------------------------------------------->>
				merge 		m:1 ApprovedMonthYear using "${path_project}/data/02-inter\usd-to-euro-ex.dta", keep (1 3) nogen
				...
				*--------------------------------------------------------------------------------------------------------------------------------->>
				...
				//same year, different loan ids and the same firm is registered with distinct sizes. For ex: firm A, loan 1-> micro, firm A, loan 2-> small.
				...
				*
				...
				replace 	duration 	  = (maturity - approvaldate)/30 			if duration == . & !missing(maturity) & ///
																						!missing(approvaldate) & (maturity - approvaldate)/30 < r(max) & (maturity - approvaldate)/30 > r(min) & (maturity - approvaldate)/30 > 0 
				compress
				save 	"${path_project}/data/02-inter\Credit Registry.dta", replace
		}		
		...
		*------------------------------------------------------------------------------------------------------------------------------------->>
		{ //the previous dataset is not a panel since the same firm can have multiple loans in the year t. 
		  //next I collapse loans by year/firm in order to have a panel and merge the loan dataset with tax registry data. 
			
			use "${path_project}/data/02-inter\Credit Registry.dta", clear
				...
				*Collapsing Loan Data by firm & year
				...
				format 		*amount* %15.2fc	
				compress
				save 	  "${path_project}/data/02-inter\Credit Registry_firm-year-level.dta", replace
		}
		*______________________________________________________________________________________________________________________________________*
		...
		*------------------------------------------------------------------------------------------------------------------------------------->>
		{
			import excel using "${path_project}/data/raw\KCGF.xlsx", allstring clear firstrow
					...
					* drop temp
					compress
					save "${path_project}/data/02-inter\KCGF.dta", replace	
			}
		*______________________________________________________________________________________________________________________________________*
