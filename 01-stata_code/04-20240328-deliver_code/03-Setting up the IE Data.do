	...
	*----------------------------------------------------------------------------------------------------------------------------------------->>
	{
		...
		*------------------------------------------------------------------------------------------------------------------------------------->>
			use "${path_project}/data/02-inter\Credit Registry_firm-year-level.dta"   if period <= 2018, clear
		...
		*------------------------------------------------------------------------------------------------------------------------------------->>
			use "${path_project}/data/02-inter\Tax Registry.dta", clear   					//Turnover = 0 or missing, how to interpret?
				drop 		group_sme
	...
	*----------------------------------------------------------------------------------------------------------------------------------------->>
	{			
				...
				*br 			fuid period turnover deathyear 				if period > deathyear & !missing(deathyear) 
				...
				gen		 	erro  = 1  					 				if  deathyear < max_year_turn					//the firm registered sales in a year after the deathyear, 
																														//which is an evidence of error. 
				...
				gen 		erro = 1 if formal == 0 & period > deathyear												//formal = 0 means that the firm is only in the credit registry //
																														//data (so the firm has a loan)
				...
				*------------------------------------------------------------------------------------------------------------------------------>>
				...
				//we are going to create a balanced panel
				...
				*------------------------------------------------------------------------------------------------------------------------------>>
				...
				replace 		main_dataset = 0	if main_dataset == .								//main_dataset = 0, rows created to input 0 to num employees, productivity and 
																										//sales for firms that closed
				...
				* Changing missing to constant caracteriscts or getting lag
				{
							...
							* Changind by mode pela moda 
							foreach var of varlist sectionid sectionid1-sectionid21 divisionid activityid municipalityid  ethnicity  {
								replace `var' =. if `var'< 0
								* nao faca para pis 
								di "Ajustando `var'"
								cap drop moda_aux
								bys fuid  `var'		: gen int moda_aux = _N
								
								* Nao podemos dar prioridade ao missing value
								cap confirm string var `var'
								if _rc ==0 	replace moda_aux = 0 if `var'==""
								else 		replace moda_aux = 0 if `var'==.
								
								bys fuid (moda_aux)	: replace `var'	= `var'[_N] if  `var' != .
							...
							sort fuid period
							cap drop sme_num
							gen 	sme_num =1  if sme =="a.1-9"
							replace sme_num =2  if sme =="b.10-49"
							replace sme_num =3  if sme =="c.50-249"
							replace sme_num =4  if sme =="d.250+"
							
							* bro fuid period sme_num
							
							bys fuid (period): replace 	   sme_num = cond(missing(sme_num),sme_num[_n-1],sme_num)
							
							replace size_creditdata = . if size_creditdata==5
							bys fuid (period): replace 	   size_creditdata = cond(missing(size_creditdata),size_creditdata[_n-1],size_creditdata)
				}
				.
				...
				* Leandro main adjusts
				{ 		
					*error
					bys fuid (period): gen  D_first_year= _n==1
				
					gen D_outlier = 0
					replace D_outlier =  2 if salaries_r	<=exp(4)
					replace D_outlier =  3 if turnover_r	<=exp(4)
					replace D_outlier =  1 if productivity_r<=exp(4) 
					
					tab period D_outlier, m
					
					*sum employees turnover_r productivity_r wages_worker_r firms_age has_credit_history ///
					*	if  D_outlier==0	& ///
					*		active == 1 & period == 2018 & main_dataset == 1
					...
					cap drop D_study_sample
					gen D_study_sample = !inlist(productivity_r ,.,0)	& ///
										 !inlist(employees	    ,.,0) 	& ///
										 !inlist(salaries_r		,.,0) 	& ///
										 !inlist(turnover_r		,.,0) 		
					...
					* To matching
					...
					gen log_prod 	 = log(productivity_r)
					gen log_turn 	 = log(turnover_r)
					gen log_wage 	 = log(wages_worker_r)
					
					foreach var of varlist log_???? {
						di as white "`var'"
						gen L1_`var' =  L1.`var' 
						gen Dm_L1_`var' = L1_`var'==.
						replace L1_`var' = 0 if Dm_L1_`var'==1
					}
					.
					
					foreach var of varlist sq_lag* {
						gen Dm_`var' 	= `var'==.
						replace `var' = 0 if Dm_`var'==1
					}
					 
					* Missing adjusts
					foreach var of varlist  *number_loans* {
						replace `var' = 0 if missing(`var')
					}
					.
				}
				...
				**
				sort 		fuid period
				compress
				save 			"${path_project}/data/03-final\firm_year_level.dta", replace
		}		
