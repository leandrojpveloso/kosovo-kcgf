	...
	**
			...
			*----------------------------------------------------------------------------------------------------------------------------->>
			{
				use 	"$data\inter\Updated KCGF Panel.dta", clear
				codebook 	businessid, detail
				local total = r(N)
				di `total'
				merge 		m:1 businessid using "$data\inter\Tax Payer and Businessid.dta"	
			...
			*We will merge KCGF data and Tax Registry to have a measure of firms' turnover, employment and productivity before and after KCGF participation
			...
			use  "$data\inter\Updated KCGF Panel.dta" if size_kcgf != 4, clear
				...
				*/
				...
				merge 	m:1 businessid					 using "$data\inter\Tax Payer and Businessid.dta", keep(3  ) nogen keepusing(tax_payer_no)
				merge 	1:1 tax_payer_no period 		 using "$data\inter\Updated Tax Registry.dta"	 , keep(1 3) nogen
				...
				*/
				...
				save 	"$data\inter\Updated IE dataset.dta", replace
			}	
		...
		*--------------------------------------------------------------------------------------------------------------------------------->>
		{ 					
			use "$data\inter\Updated IE dataset.dta" if size == 1 & period > 2015 , clear
		...
		*--------------------------------------------------------------------------------------------------------------------------------->>
			...
			*---------------------------- ------------------------------------------------------------------------------------------------->>
			...
			use "$data\inter\Updated IE dataset.dta" if size == 1, clear
		...
		*--------------------------------------------------------------------------------------------------------------------------------->>
		{		
			use "$data\inter\Updated IE dataset.dta" if (max_size == 1 | max_size == 2) , clear
