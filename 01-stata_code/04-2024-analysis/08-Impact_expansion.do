*A*
*Variables selected as good predictors of the probability of getting a KCGF loan, machine learning models
*________________________________________________________________________________________________________________________________________*
{ //Analysis performed by Simon Neumeyer in Python 

	**
	*Machine Learning Models tested: OLS, Lasso, Forest, XGboost. Each model has the vector of best predictors of the probability of having a kcgf loan.
	*Simon did the analysis considering KCGF firms with firms with no loans at all (first block of covariates)
	*									KCGF firms with firms with firms with other loans (second block of covariates)
	...
	*------------------------------------------------------------------------------------------------------------------------------------*
	...
	.
		...
		use "${path_project}/data/final\firm_year_level.dta" if inlist(group_sme,1,2,3)  , clear 
		...
		*----------------------------------------------------------------------------------------------------------------------------*
		...
		foreach size of numlist 1/50 {
			*local size 10
			* PSM 1-10
			qui psmatch2 treated, pscore(pscore) neighbor(`size')  cal(0.005) com 
			qui replace _pscore= pscore if _pscore!=.
					
			preserve
				* Dropping out of commum support
				qui gen byte D_drop_sample = (pscore==. | _support==0)
				qui 	label var D_drop_sample "without pscore or out of commum support"
				qui drop if D_drop_sample==1	
				
				qui cap drop D_expansion_program
				...
				di as white "expanding up to `size'"
				tab D_expansion_program,m
				
				ksmirnov _pscore, by(D_expansion_program)  
			restore		
		}
		.
	...
	local size 15
		...
		* Checking
		tab _weight treated  ,m
			
		* Aplicando teste de media - como nao e possivel limitar o numero de iter separamos
		pstest $controls0_xgboost, both graph
		 
		* Graph pre matching
		kdensity _pscore 			  if treated ==0							 	, plot(kdensity _pscore if treated ==1) 								clpattern(dash) legend(label(1 "Não Tratados") label (2 "Tratados") rows(1)) title("sem match") saving(no1,replace)
		kdensity _pscore			  if treated ==0 & _support==1 & _weight!=. 	, plot(kdensity _pscore if treated ==1 & _support==1 & _weight!=. ) 	clpattern(dash) legend(label(1 "Não Tratados") label (2 "Tratados") rows(1)) title("com match") saving(yes1,replace)
