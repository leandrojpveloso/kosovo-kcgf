*
	...
	*Statistics for blogpost
	{
		...
		use "${path_project}/data/final/firm_year_level.dta" 	if period == 2018 & active == 1, clear
		tab sme has_credit_history, mis
	
	
		**
		*Share of small firms, 2015
		use "${path_project}/data/final/firm_year_level.dta" 	if period == 2015 & active == 1, clear	//2015, prior to the launch of KCGF
		count
		tab sme, mis
		tab sme,
	...
	*Descriptive statistics
	{	
		...
		*Percentage of firms with turnover that report profit 
		use 	"${path_project}/data/final/firm_year_level.dta" if active == 1, clear
		...
		*Number of loan operations in 2018 and number of firms. operations> number of firms because there are firms with more than one loan. 
		use "${path_project}/data/inter/Credit Registry.dta", clear
		count
		...
		*46% of the small firms in 2018 were created after 2010.
		use "${path_project}/data/final/firm_year_level.dta" 	if  active == 1 & period == 2018 & group_sme == 2 &turnover_r_all != 0, clear
			tab birthyear	
		
		
		**
		*For firms with loans approved in 2018, what is the percentage of them with credit history.
		use 	"${path_project}/data/final/firm_year_level.dta" if active == 1 & period == 2018, clear	
		...
		*Percentage of loans by economic sector of activity.
		use 	"${path_project}/data/final/firm_year_level.dta" if active == 1 & inlist(period, 2018) & sec_activityid != . & group_sme!= 5, clear	
		...
		*Comparing micro-firms and small/medium/large firms controlling for sector of activity. 
		use 	"${path_project}/data/final/firm_year_level.dta" if active == 1 & inlist(period, 2018) & !missing(sectionid), clear
	...
	*Firms' characteristics by size 2015
	{
		use "${path_project}/data/final/firm_year_level.dta" if period == 2015 & active == 1, clear
			...
			**
			...
			iebaltab turnover_r employees productivity_r willclose_after2015 nocredit_history, ///
			cov(sectionid)  format(%12.0fc %12.2fc) grpvar(group_sme) savexlsxlsx("$output/tables/Firms' characteristics by size_2015.xls") 	rowvarlabels replace 
	...
	*--------------------------------------------------------->>>
	{	//we only have turnover reported to the banks in the KCGF dataset, so we restrict this analysis for these firms. 
		...
		**
		**
		use 	"${path_project}/data/final/firm_year_level.dta" if period == 2018 & has_kcgf == 1, clear
			keep 	has_loan period turnover_r type_firm_after2015 employees productivity_r irate_nominal group_sme
			gen 	base = 1
			
		**
		**
		append  using "${path_project}/data/inter/KCGF.dta" 	 , keep(turnover_r loanamount_r period size_kcgf employees productivity_r irate_nominal)
	...
	*Economic sector of activity by size. 
	{
	use 	"${path_project}/data/final/firm_year_level.dta" if active == 1 & inlist(period, 2018) & turnover_r_all != 0, clear
		gen id = 1
		replace group_sme = 3 if group_sme == 4
	...
	*% of firms with Loans over the years.
	{
		use 	"${path_project}/data/final/firm_year_level.dta" if active == 1 & inlist(period, 2014, 2010, 2018), clear	
	...
	**Percentage of firms by size within the years - only firms with a non-zero turnover.
	{
	use "${path_project}/data/final/firm_year_level.dta" if  active == 1 & group_sme != 5 & turnover_r_all != 0, clear
		gen 	id = 1
		replace group_sme = 3 if group_sme == 4
	...
	*We observe a decrease in the percentage of micro firms over the years. Are these micro-firms turning into small firms?
	{
	use "${path_project}/data/final/firm_year_level.dta" 	if  active == 1 & group_sme == 1 & period == 2010 & turnover_r_all != 0, clear //micro firms in 2010 with a non-zero turnover. 
		keep 	fuid
		tempfile temp
		savexls 	`temp'
	...
	*Exit rate by size. 
	{
		use "${path_project}/data/final/firm_year_level.dta" if period == 2015 & main_dataset == 1 & active == 1 & inlist(sme, "a.1-9", "b.10-49", "c.50-249"), clear
	...
	*__________________________________________________________________________________________________________________________________________*
	{	//the bigger the firm, the higher the % with loans
		...
		*----------------------------------------------------------------------------------------------------------------------------*
		*----------------------------------------------------------------------------------------------------------------------------*
		use "${path_project}/data/final/firm_year_level.dta" if active == 1 & inlist(period, 2015) & group_sme != 5 , clear
		*----------------------------------------------------------------------------------------------------------------------------*
	...
	*__________________________________________________________________________________________________________________________________________*
	{	
		...
		*----------------------------------------------------------------------------------------------------------------------------*
		*----------------------------------------------------------------------------------------------------------------------------*
		use "${path_project}/data/final/firm_year_level.dta" if active == 1 & inlist(period, 2017, 2018) , clear
		*----------------------------------------------------------------------------------------------------------------------------*
	...
	*Which are the firms' characteristics that increase their probability of having a loan approved? 
	...
	
	{
		...
		*Probability of having a loan according to the Machine Learning model run by Simon Valerio
		import 		delimited using "${path_project}/data/final/machine-learning\potential_borrowers_top10_vars.csv", clear
	...
	*Distribution in the probability of having a loan 
	{
		...
		*----------------------------------------------------------------------------------------------------------------------------*
			...
			**
			merge 1:1 fuid period using "${path_project}/data/final/firm_year_level.dta", keep(1 3) keepusing(sme) nogen
	...
	*Credit creditworthiness -> Results of the machine learning models Simon run in Python
	{
		...
		import 		delimited using "${path_project}/data/final/machine-learning\potential_borrowers.csv", clear
	...
	*Balance test between:
	...
	{
		use 	"${path_project}/data/final/firm_year_level.dta" if active == 1 & period == 2018 & main_dataset == 1, clear	
	...
	*Firms' characteristics prior to KCGF according to their access to credit between 2016-2018
	{
		use "${path_project}/data/final/firm_year_level.dta" if period == 2015 & main_dataset == 1 & active == 1 & inlist(sme, "a.1-9", "b.10-49"), clear
	...
	*Firms size according to Tax Registry. Lots of missings, undereporting might be a problem
	{	
		...
		*----------------------------------------------------------------------------------------------------------------------------*
		*----------------------------------------------------------------------------------------------------------------------------*
		use 		"${path_project}/data/final/firm_year_level.dta" if inlist(period, 2018,2017) & active == 1, clear
		*----------------------------------------------------------------------------------------------------------------------------*
		**
		**
		gen 		id = 1
	...
	*Distribution of the number of employees among firms with access to credit, only micro and small firms
	{	
		...
		*----------------------------------------------------------------------------------------------------------------------------*
		*----------------------------------------------------------------------------------------------------------------------------*
		use 	"${path_project}/data/inter/KCGF.dta"  if LoanStatus != "Canceled" & inlist(size_kcgf, 1,2), clear
		*----------------------------------------------------------------------------------------------------------------------------*
			gen 	 has_kcgf = 1
		...
		*----------------------------------------------------------------------------------------------------------------------------*
		*----------------------------------------------------------------------------------------------------------------------------*
		use 	"${path_project}/data/final/firm_year_level.dta" if active == 1  & inlist(sme, "a.1-9", "b.10-49") , clear		
		*----------------------------------------------------------------------------------------------------------------------------*
	...
	*Distribution in the number of employees of KCGF and non kcgf loans
	{
		...
		*----------------------------------------------------------------------------------------------------------------------------*
		*----------------------------------------------------------------------------------------------------------------------------*
		use 	"${path_project}/data/final/firm_year_level.dta" if active == 1  & inlist(sme, "a.1-9", "b.10-49") & period == 2015 & employees<=30, clear		
		*----------------------------------------------------------------------------------------------------------------------------*
	...
	*KCGF disbursement amount and number of contracts
	{	
		*----------------------------------------------------------------------------------------------------------------------------*
		*----------------------------------------------------------------------------------------------------------------------------*
		use "${path_project}/data/inter/KCGF.dta" if period > 2015 & LoanStatus != "Canceled", clear
		*----------------------------------------------------------------------------------------------------------------------------*
	...
	*Distribution of the amount of kcgf and non-kcgf loans. Average loan, only MSME
	{	
		...
		*----------------------------------------------------------------------------------------------------------------------------*
		*----------------------------------------------------------------------------------------------------------------------------*
		use "${path_project}/data/inter/KCGF.dta" 					if period > 2015 & LoanStatus != "Canceled" & inlist(size_kcgf, 1,2,3), clear	//not including large companies
		*----------------------------------------------------------------------------------------------------------------------------*
		...
		*----------------------------------------------------------------------------------------------------------------------------*
		*----------------------------------------------------------------------------------------------------------------------------*
		use 	"${path_project}/data/inter/Credit Registry.dta" 	if period > 2015 & fund != 1, clear														//non kcgf loans. Each row in this dataset is one contract. same firm can have several in the same year. 
		...
		*----------------------------------------------------------------------------------------------------------------------------*
				...
				**Checking the firm sales to see if the approved amount is much more than the total sales (which indicates that something might be wrong)
				merge 	m:1 fuid period using "${path_project}/data/final/firm_year_level.dta", keep (1 3) keepusing (turnover_r size sme) nogen
	...
	*Descriptive statistics of non-KCGF loans
	{
		*----------------------------------------------------------------------------------------------------------------------------*
		*----------------------------------------------------------------------------------------------------------------------------*
		use 	"${path_project}/data/inter/Credit Registry.dta" 	if period > 2015, clear							//each row is one contract. same firm can have several in the same year. 
		replace fund = 2 if fund == 3
		*----------------------------------------------------------------------------------------------------------------------------*
		
				**
				*Checking the firm sales to see if the approved amount is much more than the total sales (which indicates that something might be wrong)
				merge m:1 fuid period using "${path_project}/data/final/firm_year_level.dta", keep (1 3) keepusing (turnover_r size sme group_sme) nogen
	...
	*Balance test between Regular KCCF window and Economic Recovery Package
	{
		...
		use "${path_project}/data/inter/KCGF.dta" if period == 2021, clear
	...
	*% loans by KCGF window 2021
	{
		...
		*----------------------------------------------------------------------------------------------------------------------------*
		*----------------------------------------------------------------------------------------------------------------------------*
		use "${path_project}/data/inter/KCGF.dta" 				if period == 2021 & LoanStatus != "Canceled" & inlist(size_kcgf, 1,2,3), clear	//not including large companies
		*----------------------------------------------------------------------------------------------------------------------------*
