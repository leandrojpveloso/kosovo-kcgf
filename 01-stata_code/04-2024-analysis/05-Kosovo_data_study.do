...
* 01: KCGF panel
{
	use 	  "${path_project}/data/inter/02-KCGF-panel.dta", clear
	
	tab period
	tab period if id_tax!=""
	
	* Keeping tax information
	keep if id_tax!=""
	
	di `=_N' 
	merge m:1 id_tax period 		 using "${path_project}/data/inter/03-Tax Registry.dta"	 , keep(1 3) 
...
{ 
	...
	bys   	id_firm: egen max_size = max(size) //if we want to include a control in the Staggered DiD proposed by Woodrigde, the control shoulc be time invariant
	...
	order 	id_tax id_firm period
	sort 			   	 id_firm period


	xtset   id_firm_num period 
	...
	bys 	id_firm: egen has_turn_beforekcgf = max(has_turn)
	drop    has_turn

	gen 	has_turn = turnover > 0 & !missing(turnover) & period >= first_kcgf //has turnover after during or after kcgf participation
	bys 	id_firm: egen has_turn_duringkcgf = max(has_turn)
	drop    has_turn

	sort 	id_firm  period
	*br 		id_firm period loanamount		   first_kcgf t treated_before2022 after_treatment DT turnover turnover_kcgf has_turn_beforekcgf  has_turn_duringkcgf
...
label 		var export_tx					"Firm exports"
 	...
 	compress
	save 	 "${path_project}/data/inter/05-impact_evaluation_dataset.dta", replace
	...
	{			
				...
				*------------------------------------------------------------------------------------------------------------------------------>>
				...
				foreach 	var of varlist sectionid sectionid1-sectionid21 divisionid activityid size size_creditdata municipalityid  ethnicity {
					replace `var' = `var'[_n-1] 	if main_dataset == 0 	& `var'[_n-1] != .  & fuid[_n] == fuid[_n-1]		//to run the regressions with the exited firms, 
																																//we need the variables size, economic sector, and
																																//municipality to use as control. 
				}
